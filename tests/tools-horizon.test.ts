...
describe("MCP tool contract matrix", () => {
  ...
  it("requires explicit milestone approval when auto-approval is disabled and then finalizes consistently", async () => {
    ...
    await server.callTool("horizon_observe_receipt", { sessionId: "approval", parentSessionId: "approval-parent-worker", featureId: "f1", childRunId: "approval-worker", receiptId, summary: "worker complete" })
    await server.callTool("horizon_begin_auditor", { sessionId: "approval", featureId: "f1", childRunId: "approval-auditor" })
    completeDispatch(projectRoot, "approval-parent-auditor", "approval", "auditor", "approval-auditor")
    await server.callTool("horizon_record_audit", { sessionId: "approval", parentSessionId: "approval-parent-auditor", featureId: "f1", childRunId: "approval-auditor", verdict: "accept", summary: "accepted" })
