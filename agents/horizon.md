...
4. **Execute.** Work one feature at a time. Dispatch exactly one `parallax-claudecode:horizon-worker` with the first prompt line `HORIZON_DISPATCH {"sessionId":"<horizon-session>","featureId":"<feature>"}`. The description must identify this as foreground Horizon work. Never overlap children. Require surgical changes, targeted checks, and a schema-v2 receipt.
