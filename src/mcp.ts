...
export class ParallaxMcpServer {
  ...
  async handleRequest(request: unknown): Promise<Record<string, unknown> | null> {
    ...
    if (method === "initialize") {
      return { jsonrpc: "2.0", id, result: { protocolVersion: MCP_PROTOCOL_VERSION, capabilities: { tools: { listChanged: false } }, serverInfo: { name: "parallax-claudecode", version: "0.2.4" } } }
    }
